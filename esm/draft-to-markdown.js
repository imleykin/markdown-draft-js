...
var MARKDOWN_STYLE_CHARACTERS = /(\*|_|~|\\|`)/; // I hate this a bit, being outside of the function’s scope
...
function renderBlock(block, index, rawDraftObject, options) {
  ...
  var type = block.type; // draft-js emits empty blocks that have type set… don’t style them unless the user wants to preserve new lines
  ...
  Array.from(block.text).some(function (character, characterIndex) {
    ...
    if (block.type !== 'code-block' && escapeMarkdownCharacters) {
      ...
      if (insideInlineCodeStyle) {// Todo - The syntax to escape backtics when inside backtic code already is to use MORE backtics wrapping.
        ...
        character = character.replace(MARKDOWN_STYLE_CHARACTERS, '\\$1'); // Special escape logic for blockquotes and heading characters
